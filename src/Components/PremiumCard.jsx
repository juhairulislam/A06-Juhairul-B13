...
const PremiumCard = ({ data, carts, setCarts }) => {
    ...
    return (
        <div className=' shadow rounded-xl  p-2 md:p-5 mb-5 hover:shadow-2xl'>
            ...
            <button type="button" onClick={() => clickBuy(data)} className={`${isBuy ? 'bg-linear-to-r from-[#4F39F6]  to-[#9514FA]  text-white' : 'bg-green-400 text-white'} h-8 btn btn-xs sm:btn-sm md:btn-md lg:btn-lg xl:btn-xl w-full  rounded-xl p-5 font-semibold text-[15px] my-4`}>{isBuy ? "Buy Now" : "Added To cart"}</button>
